	...
	switch (newItem.Status.S) {
		case "PENDING_INTERVIEW":
			if (newItem.InterviewerEmail.S) {
				email = buildEmail({
					...
					body: commonBody("Interview Assignment"),
				});
			}
			break;
		case "INTERVIEW_SCHEDULED":
			email = buildEmail({
				...
				body: commonBody("Interview Scheduled").replace(
					'<div class="highlight">',
		...
		case "INTERVIEW_COMPLETED":
			email = buildEmail({
				...
				body: commonBody("Interview Completed"),
			});
			break;
		case "OFFER_MADE":
			email = buildEmail({
		...
		case "OFFER_ACCEPTED":
			email = buildEmail({
				...
				body: commonBody("Offer Accepted"),
			});
			break;
		default:
